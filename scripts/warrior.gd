...
# The summon grace. State.SUMMONING locks the warrior out: _unhandled_input early-returns
# on it (so no attack, dash or Song) and _handle_movement zeroes velocity. He cannot dodge
# or step during it — being damageable through a window he cannot act in would lose runs
# to an animation. take_damage() therefore ignores hits
