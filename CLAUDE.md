...
- **Maps are per-run randomized** (2026-06-30): every ring world + enemy layout derives RNG from `GameState.ring_seed(local)`, offset by a per-summon `GameState.run_seed` rolled in `main.gd._ready()`. `run_seed == 0` (and `GameState.lock_seed = true`) reproduces the original pinned layouts byte-for-byte for the look reference / tests — for terrain and enemy pockets. **Creature placement is exempt since the roster landed (2026-07-28):** `creature_roster.gd` replaced the fixed Threshold offsets / fixed herd home, so a locked seed pins creatures *deterministically* but not to the pre-roster spots. Previously every run was seed-pinned identical.
